...
contract WalletTemplateOne is ERC1155MockReceiver,IWalletTemplate {
    ...
    /// @dev Init wallet only once
    /// @param _owners List of initial owners.
    /// @param _required Number of required confirmations.
    function initWallet(address[] calldata _owners, uint _required)
        external
        validRequirement(_owners.length, _required)
    {
        require(owners.length==0,'WalletTemplateOne: has been initial');
    ...
    function removeOwner(address owner)
        ...
        ownerExists(owner)
    {
        require(owners.length > 1,"WalletTemplateOne: at least one owner");
        isOwner[owner] = false;
